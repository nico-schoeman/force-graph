import { World } from 'ecsy/build/ecsy.module.js';
import { Position, Velocity, Link, Anchor } from './components.js';
import { AttractSystem, RepulseSystem, MoveSystem } from './systems.js';
...
force_graph.prototype.addLink = async function(fromNode, toNode) {
	let link = await this.world.createEntity().addComponent(Link, { from: fromNode.id, to: toNode.id });
	return link;
};
