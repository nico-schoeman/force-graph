import { System, Not } from 'ecsy/build/ecsy.module.js';
...
export class AttractSystem extends System {
	execute(delta, time) {
		this.queries.links.results.forEach(linkEntity => {
			...
			let nodesLinked = this.queries.nodes.results.filter(node => {
				return node.id === link.from || node.id === link.to;
			});
...
export class RepulseSystem extends System {
	...
	calculateVector(myPos, otherPos) {
		...
		let vector =
			myPos.x != otherPos.x && myPos.y != otherPos.y
				? otherPos.subtract(myPos)
